...
#define IDLE_Current	46 // rund 10mA bei 0,01Ohm Shunt
	...
	if ( timer == 3 ) {
		...
		if ( AC1.getcount() > 3 && 
			...
			if ( _CU > _CUR ) {									// Laden
				...
				if ( _A1 < _A2 && _A1 < _A3 ) {
					_PORTB &= ~(1<<_B_ACCU1_ON);
				}
				if ( _A2 < _A1 && _A2 < _A3 ) {
					_PORTB &= ~(1<<_B_ACCU2_ON);
				}
				if ( _A3 < _A1 && _A3 < _A2 ) {
					_PORTB &= ~(1<<_B_ACCU3_ON);
				}
				if ( _A1 == _A2 && _A1 < _A3 ) {
					_PORTB &= ~(1<<_B_ACCU1_ON);
					_PORTB &= ~(1<<_B_ACCU2_ON);
				...
				if ( _A1 >= _A2 && _A1 >= _A3 ) {
					_PORTB &= ~(1<<_B_ACCU1_ON);
					_PORTB |= (1<<_B_ACCU2_ON)|(1<<_B_ACCU3_ON);
				}
				if ( _A2 > _A1 && _A2 > _A3 ) {
					_PORTB &= ~(1<<_B_ACCU2_ON);
					_PORTB |= (1<<_B_ACCU1_ON)|(1<<_B_ACCU3_ON);
				}
				if ( _A3 > _A1 && _A3 > _A2 ) {
					_PORTB &= ~(1<<_B_ACCU3_ON);
					_PORTB |= (1<<_B_ACCU1_ON)|(1<<_B_ACCU2_ON);
				}
			}
		}
	if ( (_PORTB & 0b00000111) == 0b00000111 )
		_PORTB &= 0b11111110;
